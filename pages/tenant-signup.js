export default function TenantSignup() {
  return (
    <main style={{ background: '#000', color: '#fff', minHeight: '80vh', padding: '90px 20px', textAlign: 'center' }}>
      ...
      <p style={{ color: '#BFBFBF' }}>Register to apply, schedule tours, and receive matching recommendations.</p>
      <form style={{ maxWidth: 520, margin: '24px auto', display: 'grid', gap: 12, textAlign: 'left' }}>
        <input style={i} placeholder="Full Name" />
        <input style={i} placeholder="Email" type="email" />
        <input style={i} placeholder="Preferred Location" />
        <input style={i} placeholder="Bedrooms Needed" />
        <label style={{ color: '#C0C0C0' }}><input type="checkbox" /> I acknowledge G & G is a marketing & tenant acquisition service only.</label>
        <button style={b}>Register</button>
      </form>
    </main>
  );
}
